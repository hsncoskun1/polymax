...
Last updated: v0.5.23 (2026-04-01) · Total automated tests: **594** (+ 1 live-skipped)
...
| v0.5.23 | Regression Tier Contract Lock — Status B: tier mantığı dağınık, executable contract eksik; added docs/testing/regression_tiers.md (Smoke/Standard/Full definitions, executable pytest commands, change-type decision table, live exclusion rules); 41 new tier contract tests (A–F); 594 total tests; 444 regression scenarios | Smoke tier defined (health+domain+services+api, ~185 tests, <1s); Standard tier defined (all automated, ~553 tests); Full tier defined (Standard+live); live tests explicitly outside required path; change-type minimum tier table; tier doc consistent with all existing docs | All prior contracts still hold | — |
...
| Snapshot Refresh Trigger Policy | 27 | 27 | 0 | 0 |
| **Total** | **360** | **359** | **0** | **1 live-skipped** |
